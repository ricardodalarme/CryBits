...
{
    /// <summary>Read all tile metadata from disk and return the populated array.</summary>
    public static Tile[] ReadAll()
    {
        var list = new Tile[Textures.Tiles.Count];
        for (byte i = 1; i < list.Length; i++) list[i] = Read(i, list);
        return list;
    }

    private static Tile Read(byte index, Tile[] list)
    {

...
class Player
{
    ...
    private static void Move(Game.Directions Direction)
    {
        ...
        if (Me.Movement != Game.Movements.Stopped) return;
        ...
        if (Map.Tile_Blocked(Me.Map_Num, Me.X, Me.Y, Direction)) return;

        // Define a velocidade que o jogador se move
        if (Keyboard.IsKeyPressed(Keyboard.Key.LShift) && Graphics.RenderWindow.HasFocus())
            Me.Movement = Game.Movements.Moving;
        else
            Me.Movement = Game.Movements.Walking;
        ...
        switch (Direction)
        {
            case Game.Directions.Up: Me.Y2 = Game.Grid; Me.Y -= 1; break;
            case Game.Directions.Down: Me.Y2 = Game.Grid * -1; Me.Y += 1; break;
            case Game.Directions.Right: Me.X2 = Game.Grid * -1; Me.X += 1; break;
            case Game.Directions.Left: Me.X2 = Game.Grid; Me.X -= 1; break;
        }
    }

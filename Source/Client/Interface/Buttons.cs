...
public class Buttons
{
    ...
    public class Events
    {
        ...
        {
            SFML.Graphics.Texture Texture = Graphics.Tex_Button[Tool.Texture_Num];

            // Somente se necessário
            if (!Tool.IsAble) return;
            if (!Tools.IsAbove(new Rectangle(Tool.Position, Graphics.TSize(Texture)))) return;

            // Altera o estado do botão
            Audio.Sound.Play(Audio.Sounds.Click);
            Tool.State = States.Above;

            // Executa o evento
            Execute(Tool.Name);
        }

        public static void MouseDown(MouseEventArgs e, Structure Tool)
        {
            SFML.Graphics.Texture Texture = Graphics.Tex_Button[Tool.Texture_Num];
            ...
            if (!Tools.IsAbove(new Rectangle(Tool.Position, Graphics.TSize(Texture))))
                return;

            // Altera o estado do botão
            Tool.State = States.Click;
        }

        public static void MouseMove(MouseEventArgs e, Structure Tool)
        {
            SFML.Graphics.Texture Texture = Graphics.Tex_Button[Tool.Texture_Num];

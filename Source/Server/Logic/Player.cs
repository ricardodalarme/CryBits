...
class Player
{
    ...
    public class Character_Structure
    {
        ...
        private void Attack_Player(byte Victim)
        {
            short Attack_Damage;
            short Next_X = X, Next_Y = Y;
            ...
            if (Map_Num != Character(Victim).Map_Num) return;
            if (Character(Victim).X != Next_X || Character(Victim).Y != Next_Y) return;
            ...
            // Cálculo de dano
            Attack_Damage = (short)(Damage - Character(Victim).Player_Defense);
            ...
            if (Attack_Damage > 0)
            {
                ...
                if (Attack_Damage < Character(Victim).Vital[(byte)Game.Vitals.HP])
                {
                    Character(Victim).Vital[(byte)Game.Vitals.HP] -= Attack_Damage;
                ...
                else
                {
                    ...
                    GiveExperience(Character(Victim).Experience / 10);

                    // Mata a vítima
                    Character(Victim).Died();
                }
            }
            else
                // Demonstra o ataque aos outros jogadores
        ...
        private void Attack_NPC(byte Victim)
        {
            short Attack_Damage;
            short Next_X = X, Next_Y = Y;
            Lists.Structures.Map_NPCs Map_NPC = Lists.Temp_Map[Map_Num].NPC[Victim];

            // Define o azujelo a frente do jogador
            Map.NextTile(Direction, ref Next_X, ref Next_Y);

            // Verifica se a vítima pode ser atacada
            if (Map_NPC.X != Next_X || Map_NPC.Y != Next_Y) return;
            ...
            switch ((NPC.Behaviour)Lists.NPC[Map_NPC.Index].Behaviour)
            {
                ...
                case NPC.Behaviour.ShopKeeper: Shop_Open(Lists.NPC[Map_NPC.Index].Shop); return;
            }

            // Define o alvo do NPC
            Lists.Temp_Map[Map_Num].NPC[Victim].Target_Index = Index;
            Lists.Temp_Map[Map_Num].NPC[Victim].Target_Type = (byte)Game.Target.Player;

            // Tempo de ataque 
            Lists.Temp_Player[Index].Attack_Timer = Environment.TickCount;

            // Cálculo de dano
            Attack_Damage = (short)(Damage - Lists.NPC[Map_NPC.Index].Attribute[(byte)Game.Attributes.Resistance]);
            ...
            if (Attack_Damage > 0)
            {
                ...
                if (Attack_Damage < Map_NPC.Vital[(byte)Game.Vitals.HP])
                {
                    Lists.Temp_Map[Map_Num].NPC[Victim].Vital[(byte)Game.Vitals.HP] -= Attack_Damage;
                ...
                {
                    // Experiência ganhada
                    GiveExperience(Lists.NPC[Map_NPC.Index].Experience);

                    // Reseta os dados do NPC 
                    NPC.Died(Map_Num, Victim);
                }
            }
            else
                // Demonstra o ataque aos outros jogadores
        ...
        private void Party_SplitXP(int Value)
        {
            ...
            for (byte i = 0; i < Lists.Temp_Player[Index].Party.Count; i++)
            {
                Difference = Math.Abs(Level - Character(Lists.Temp_Player[Index].Party[i]).Level);
            ...
            for (byte i = 0; i < Lists.Temp_Player[Index].Party.Count; i++)
            {
                ...
                Character(Lists.Temp_Player[Index].Party[i]).CheckLevelUp();

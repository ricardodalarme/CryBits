...
public sealed class ShopSystem : ISimulationSystem
{
    ...
    private void Buy(World world, Tick tick, EntityId entityId, short shopSoldIndex)
    {
        ...
        if (currencySlot == null || inv.Slots[currencySlot.Value].Amount < shopSold.Price)
        {
            tick.Events.Emit(new ChatMessageEvent(tick.TickNumber, entityId,
                "You don't have enough money to buy the item.", ChatColors.Red));
            return;
        }

        if (inv.CountFreeSlots() == 0 && inv.Slots[currencySlot.Value].Amount > shopSold.Price)
        {
            tick.Events.Emit(new ChatMessageEvent(tick.TickNumber, entityId, "You don't have space in your bag.",
                ChatColors.Red));
            return;
        }
        ...
        tick.Events.Emit(new ChatMessageEvent(tick.TickNumber, entityId,
            "You bought " + shopSold.Price + "x " + soldItemName + ".", ChatColors.Green));
    }
    ...
    private void Sell(World world, Tick tick, EntityId entityId, byte inventorySlotIndex, short amount)
    {
        ...
        if (buy == null)
        {
            tick.Events.Emit(new ChatMessageEvent(tick.TickNumber, entityId, "The store doesn't buy this item",
                ChatColors.Red));
            return;
        }
        ...
        tick.Events.Emit(new ChatMessageEvent(tick.TickNumber, entityId,
            "You sold " + amount + "x " + soldItemName + " for " + (buy.Price * amount) + ".", ChatColors.Green));

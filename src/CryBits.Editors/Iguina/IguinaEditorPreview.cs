...
internal sealed class IguinaEditorPreview : IDisposable
{
    ...
    public IguinaEditorPreview(int width, int height)
    {
        ...
        var defaultFont = File.Exists(fontPath) ? new Font(fontPath) : new Font(Path.Combine(themePath, "Textures", "UI.png"));
        _renderer = new SfmlRenderer(_target, themePath, defaultFont);
        var input = new StubInputProvider();

        var sPath = System.IO.Path.Combine(themePath, "system_style.json");
    ...
    public void Draw()
    {
        _target.Clear(SFML.Graphics.Color.Black);
        _uiSystem.Draw();
        _target.Display();
    }

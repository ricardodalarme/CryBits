...
internal static class UIRenderer
{
    ...
    private static void TextBox(TextBox tool)
    {
        ...
        Renderer.DrawBox(Textures.TextBox, 3, tool.Position, new Size(tool.Width, Textures.TextBox.ToSize().Height));
    ...
    private static void Bars(Panel tool)
    {
        ...
        Renderer.DrawText(Player.Me.Experience + "/" + Player.Me.ExpNeeded, tool.Position.X + 76, tool.Position.Y + 43,
    ...
    private static void Information(Panel tool)
    {
        ...
        Renderer.Draw(Textures.Items[item.Texture], new Rectangle(tool.Position.X + 9, tool.Position.Y + 21, 64, 64));
        ...
        for (byte i = 0; i < data.Count; i++) Renderer.DrawText(data[i], positions[i].X, positions[i].Y, Color.White);
    }
    ...
    private static void MenuCharacter(Panel tool)
    {
        ...
        Renderer.Draw(Textures.Faces[Player.Me.TextureNum], new Point(tool.Position.X + 82, tool.Position.Y + 37));

        Renderer.DrawText("Strength: " + Player.Me.Attribute[(byte)Attribute.Strength], tool.Position.X + 32,
            tool.Position.Y + 146, Color.White);
        Renderer.DrawText("Resistance: " + Player.Me.Attribute[(byte)Attribute.Resistance], tool.Position.X + 32,
            tool.Position.Y + 162, Color.White);
        Renderer.DrawText("Intelligence: " + Player.Me.Attribute[(byte)Attribute.Intelligence], tool.Position.X + 32,
        ...
        Renderer.DrawText("Points: " + Player.Me.Points, tool.Position.X + 14, tool.Position.Y + 228, Color.White);

        for (byte i = 0; i < (byte)Equipment.Count; i++)
            if (Player.Me.Equipment[i] == null)
                Renderer.Draw(Textures.Equipments, tool.Position.X + 7 + i * 34, tool.Position.Y + 247, i * 34, 0, 32,
                    32);
            else
